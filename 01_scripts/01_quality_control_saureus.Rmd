...
1. Look at your library-size barplot and pick the tallest and the shortest bar. Predict, before you scroll any further: will each of those samples get a size factor above or below 1.0 in the Estimate Size Factors table further down? Check your prediction when you get there, and note any sample where it fails. Then the biological question: if a biofilm sample had twice the total counts of a planktonic sample, would that mean biofilm cells were making twice as much RNA — why or why not?
:::
...
4. You have now seen two views of the same samples: the distance heatmap and this PCA. Commit as a group to one claim — 'these samples are organised first by time point' or 'first by lifestyle' — and point to the specific block in your heatmap and the specific axis in your PCA that support it. Where the two plots seem to disagree, reconcile them (the chapter tells you, under Detecting Outliers, that they are not computed from the same set of genes). Biologically: are a 5 h biofilm and a 24 h biofilm the same state, and what in your plots says so?
:::
...
6. The last row of the summary table asks you to record a time-point decision. Decide now, as a group, which time point you will carry into script 02, and defend the choice in one sentence that cites at least two numbers from your own outputs (the % variance on a PCA axis, the visible separation, the replicate counts in your design table). Then state the price: by keeping only that time point and comparing biofilm to planktonic as two groups, what biofilm biology do you become blind to?
:::
