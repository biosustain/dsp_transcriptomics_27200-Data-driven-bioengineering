...
:::boxy
- `results/human/DE_treatment_vs_control_all.tsv` — all tested genes with shrunken LFC
- `results/human/DE_treatment_vs_control_significant.tsv` — significant DE genes (padj < 0.05, |LFC| >= 1)
:::
...
:::boxy
**How to read these tables.** Four columns carry the meaning:

- **`term_size`** — how many genes the term contains in total. Very large terms ("signaling", 3,700 genes) are vague; very small ones (4 genes) are fragile.
- **`intersection_size`** — how many of *your* query genes fall in it. This is the actual overlap.
- **`p_value`** — already corrected by g:Profiler (g:SCS), so read it as an adjusted p-value.
- **`source`** — which database the term came from (GO:BP, KEGG, REAC).

A trustworthy hit has a decent overlap, a term size that is specific enough to mean something, and biology that makes sense for the treatment.
:::
...
multiple-testing correction** (padj < 0.05), even though ORA returned dozens of enriched terms. This is not
a contradiction — the two methods ask different questions:

- **ORA** asks whether the ~700 *significant* DE genes are concentrated in a term. They are — mostly in
  broad developmental and signalling terms, with cytokine/immune signalling among the **down**-regulated
  genes and adhesion, cytoskeleton and metal-handling terms among the up-regulated ones — so ORA lights up.
- **GSEA** asks whether an *entire* gene set shifts coherently across the full ranked list (every gene that
  DESeq2 tested, printed above). The dexamethasone response here is carried by a few hundred
  strongly-changed genes scattered across many pathways, rather than whole pathways moving together — so no
  set reaches the coordinated shift GSEA rewards.

This is a real result, not a failure: **the glucocorticoid effect here is gene-specific rather than
pathway-coherent**, which is exactly the situation where ORA is the more sensitive tool. The nominal
(uncorrected) sets below are worth reading as hypotheses only — note how mixed they are, and that they do
not simply restate the ORA themes. Treat them as "GSEA could not settle this", not as findings.
:::
...
**ORA returns dozens of corrected-significant terms, while GSEA finds no set that survives
...
- **ORA** tests whether the ~700 genes that individually passed significance are *concentrated* in a
  term. They are — in broad developmental and signalling terms, plus cytokine/immune signalling on the
  down-regulated side, consistent with an anti-inflammatory drug — so ORA reports corrected-significant
  enrichment.
- **GSEA** tests whether an *entire* gene set moves *coherently* across the full ranked list. The
  ...
  not by whole pathways shifting together, so no set reaches significance after correction. The nominal
  top hits are a mixed bag and should be treated as untested hypotheses, not as a second opinion that
  happens to agree.
...
| GSEA gene sets | MSigDB Reactome via `msigdbr` | Curated, detailed pathways covering signalling and immune processes |
| GSEA rank metric | sign(LFC) x -log10(pvalue) | Encodes direction and significance; robust to lowly expressed gene noise |
| Primary result | ORA (g:Profiler) | Dozens of corrected-significant terms; cytokine/immune signalling among the down-regulated genes |
| GSEA outcome | No padj-significant sets | Effect is gene-specific, not pathway-coherent; nominal sets are hypotheses only |

Enrichment results are saved and ready for biological interpretation and reporting. The primary,
statistically-supported findings are the ORA terms — broad developmental and signalling programmes, with
cytokine and immune signalling **down** in the treated cells, consistent with the anti-inflammatory
glucocorticoid biology described by Himes *et al.* GSEA is reported as a secondary check that, here, cannot
